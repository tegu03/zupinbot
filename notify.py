"""Telegram notifications v3 (HTML + emoji sebagai sinyal STATUS, bukan dekorasi).
Aturan kejujuran tampilan:
  - Order gagal tidak boleh terlihat seperti sukses.
  - Limit yang resting BUKAN "terisi" — dilaporkan apa adanya + status watcher.
  - conf ditampilkan untuk sinyal ARAH; untuk NO-TRADE hanya jika bermakna (>0)."""
import httpx
...
def format_trade(decision, account, exec_result):
    ...
    if d.get("dry_run") or e.get("dry_run"):
        ...
        if e.get("entry_status") == "resting":
            lines.append(f"⏳ <b>Limit dipasang — BELUM terisi</b> · tx <code>{_esc(_short(e.get('tx_hash')))}</code>")
            lines.append("• Dibatalkan otomatis siklus berikutnya jika tak tersentuh")
        else:
        ...
        if prot.get("deferred"):
            if prot.get("mode") == "watcher":
                lines.append(f"🛡️ SL/TP menunggu fill — <b>watcher aktif</b> "
                             f"(cek tiap {int(prot.get('poll_sec') or 20)} dtk; OCO dipasang begitu limit terisi)")
            else:
...
def format_guardian(actions, phase=""):
    icon = {"PROTECTED": "✅", "STILL_NAKED": "⚠️", "UNVERIFIED": "❓",
            "NAKED_NO_ENTRY_PRICE": "⚠️", "SKIPPED_OTHER_MARKET": "❓"}
...
def format_online():
    ...
    return (f"🤖 <b>Zupin Bot ONLINE</b> · {mode} · loop tiap {CONFIG.loop_minutes} menit\n"
            f"• Data: Lighter (candles+funding) + Bybit (OI, L/S) + F&amp;G\n"
            f"• Gerbang: conf ≥ {CONFIG.min_confidence:g}% · R:R ≥ {CONFIG.min_rr:g} · "
            f"stop ≥ {CONFIG.min_stop_pct * 100:.2f}% · trend-only (ranging/chop = no-trade)\n"
            f"• Kill switch: -{CONFIG.daily_loss_limit_pct * 100:g}%/hari → tutup semua + pause s.d. "
...
def format_stale_cancel(results):
    ok = sum(1 for r in results if r.get("ok"))
    return (f"🧹 <b>Limit entry basi dibatalkan</b> · {ok}/{len(results)} order\n"
            "• Tesis siklus lama tidak dibawa ke siklus baru")


def format_kill_switch(account, res, resume_str):
    closed = [c for c in (res.get("closed") or []) if c.get("ok")]
    flat = res.get("flat")
    lines = [_header(account), "",
             "🚨 <b>KILL SWITCH ACTIVATED</b>",
             f"• Daily loss: {_sgn(account.get('daily_pnl_pct'))}% (limit: -{CONFIG.daily_loss_limit_pct * 100:.2f}%)",
             f"• Posisi ditutup: {len(closed)} · order entry dibatalkan: {len(res.get('canceled_entries') or [])}",
             f"• Status flat: {'✅ ya' if flat else ('❓ belum terkonfirmasi' if flat is None else '❌ TIDAK — cek manual')}",
             f"• Bot PAUSE sampai {resume_str}"]
    lines += ["", "<i>Bukan nasihat finansial · Testnet</i>"]
    return "\n".join(lines)
...
def format_profit_lock(account):
    lines = [_header(account), "",
             ...
             "• Entry baru dikunci sampai besok · posisi berjalan tetap dikelola OCO/guardian",
             "", "<i>Bukan nasihat finansial · Testnet</i>"]
    return "\n".join(lines)
