"""Entry point v3. Pipeline per siklus:
  account -> guardian -> kill/profit latch -> stale-limit sweep -> position guard
  -> market data (Lighter+Bybit) -> MSE -> PTE -> Risk Governor -> execute -> Telegram

KILL SWITCH (Bug #4, spek briefing Section 10):
  daily <= -3%  ->  tutup SEMUA posisi (reduce-only) + batalkan order
                ->  Telegram alert  ->  PAUSE sampai RESUME_HOUR UTC (0 = 07:00 WIB)
                ->  baseline reset otomatis saat hari UTC berganti  ->  RESUMED.
  Fail-safe: jika flatten TIDAK terkonfirmasi, bot TIDAK tidur panjang — loop normal
  berlanjut (guardian tetap menjaga, latch memblokir entry baru) + minta cek manual.
PROFIT LOCK: daily >= +target -> entry baru dikunci sampai besok (posisi berjalan
  tetap dikelola OCO/guardian). Fungsi: disiplin berhenti saat hijau. Bukan ramalan.
"""
import time
import calendar
import asyncio
...
def _seconds_until_resume():
    """Detik sampai RESUME_HOUR UTC berikutnya (0 = 00:00 UTC = 07:00 WIB)."""
    now = int(time.time())
    tm = time.gmtime(now)
...
async def kill_flow(ex: Exchange, account):
    """Latch -> flatten semua -> alert -> tidur sampai resume -> RESUMED."""
    latch_kill(float(account.get("daily_pnl_pct") or 0.0))
    res = {"flat": True, "closed": [], "canceled_entries": []}
    if not CONFIG.dry_run and ex.signer is not None:
    ...
    fresh = await ex.get_account()   # hari UTC baru -> baseline di-reset otomatis
    fresh.pop("_raw", None)
...
async def run_cycle(ex: Exchange):
    ...
    # GUARDIAN dulu: posisi terbuka apa pun harus terproteksi, selalu.
    ...
    if not CONFIG.dry_run:
        # KILL SWITCH — dicek di awal siklus, bukan hanya lewat jalur LLM.
        if kill_latched():
        ...
        if dp <= -CONFIG.daily_loss_limit_pct * 100:
            ...
            return
        # PROFIT LOCK — hari sudah hijau melewati target: kunci.
        if profit_latched():
    ...
    # STALE-ENTRY SWEEP: limit tak terisi dari siklus lalu = tesis kadaluarsa.
    if CONFIG.cancel_stale_entries and not CONFIG.dry_run and ex.signer is not None:
        stale = await ex.cancel_entry_orders()
        if stale:
            log.info("stale entry orders canceled: %s", stale)
            await send(format_stale_cancel(stale))

    # POSITION GUARD: satu posisi pada satu waktu.
    if CONFIG.block_if_position_open and not CONFIG.dry_run:
    ...
    # Belt-and-suspenders: governor juga bisa memicu kill/profit flow.
    if not CONFIG.dry_run:
    ...
    if decision["approved"]:
        ...
        # Guardian pass kedua: pastikan posisi yang baru terisi benar terproteksi.
...
async def main():
    ...
    # Restart di tengah hari yang sudah kena kill switch -> langsung tidur sampai resume.
